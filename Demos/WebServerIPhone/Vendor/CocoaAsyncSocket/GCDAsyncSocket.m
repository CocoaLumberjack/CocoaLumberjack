...
{
    if (bufferPtr) {
        *bufferPtr = readPointer;
    }

    if (availableBytesPtr) {
        *availableBytesPtr = writePointer - readPointer;
    }
}
...
{
    if (bufferPtr) {
        *bufferPtr = writePointer;
    }

    if (availableSpacePtr) {
        *availableSpacePtr = preBufferSize - (writePointer - readPointer);
    }
}
...
- (void)doReadTimeout
{
    ...
    if (delegateQueue && [delegate respondsToSelector:@selector(socket:shouldTimeoutReadWithTag:elapsed:bytesDone:)]) {
        ...
        dispatch_async(delegateQueue, ^{ @autoreleasepool {
                                             ...
                                             timeoutExtension = [theDelegate socket:self
                                                           shouldTimeoutReadWithTag:theRead->tag
...
- (void)doWriteTimeout
{
    ...
    if (delegateQueue && [delegate respondsToSelector:@selector(socket:shouldTimeoutWriteWithTag:elapsed:bytesDone:)]) {
        ...
        dispatch_async(delegateQueue, ^{ @autoreleasepool {
                                             ...
                                             timeoutExtension = [theDelegate socket:self
                                                          shouldTimeoutWriteWithTag:theWrite->tag
    ...
    #if TARGET_OS_IPHONE
    {
        ...
        if (sslMinLevel || sslMaxLevel) {
            ...
            SSLProtocol (^ sslProtocolForString)(NSString *) = ^SSLProtocol (NSString *protocolStr) {
                if ([protocolStr isEqualToString:@"kSSLProtocol3"]) {
                    return kSSLProtocol3;
                }

                if ([protocolStr isEqualToString:@"kTLSProtocol1"]) {
                    return kTLSProtocol1;
                }

                if ([protocolStr isEqualToString:@"kTLSProtocol11"]) {
                    return kTLSProtocol11;
                }

                if ([protocolStr isEqualToString:@"kTLSProtocol12"]) {
    ...
    if ([address length] >= sizeof(struct sockaddr)) {
        ...
        if (sockaddrX->sa_family == AF_INET) {
            if ([address length] >= sizeof(struct sockaddr_in)) {
                ...
                if (hostPtr) {
                    *hostPtr = [self hostFromSockaddr4:&sockaddr4];
                }

                if (portPtr) {
                    *portPtr = [self portFromSockaddr4:&sockaddr4];
                }
            ...
            if ([address length] >= sizeof(struct sockaddr_in6)) {
                ...
                if (hostPtr) {
                    *hostPtr = [self hostFromSockaddr6:&sockaddr6];
                }

                if (portPtr) {
                    *portPtr = [self portFromSockaddr6:&sockaddr6];
                }
